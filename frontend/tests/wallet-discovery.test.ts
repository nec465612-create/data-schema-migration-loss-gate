...
describe("wallet discovery", () => {
  it("uses exact RDNS identities and retains late EIP-6963 announcements", async () => {
    ...
    const metamaskProvider = { request: async () => [] };
    const rabbyProvider = { request: async () => [] };
    target.addEventListener("eip6963:requestProvider", () => {
      announce(target, { uuid: "metamask", name: "Untrusted Name", rdns: "io.metamask" }, metamaskProvider);
      announce(target, { uuid: "metamask-alias", name: "MetaMask", rdns: "io.metamask" }, metamaskProvider);
      announce(target, { uuid: "rabby", name: "Rabby Wallet", rdns: "io.rabby" }, rabbyProvider);
      announce(target, { uuid: "unknown", name: "MetaMask", rdns: "com.example.wallet" }, {});
      announce(target, { uuid: "missing", name: "OKX Wallet", rdns: "" }, {});
      announce(target, { uuid: "invalid", name: "OKX Wallet", rdns: "com.okex.wallet" }, { request: "not-callable" });
    });
    ...
    announce(target, { uuid: "okx", name: "Spoofed Name", rdns: "com.okex.wallet" }, { request: async () => [] });
  ...
  it("maps one exact legacy flag and rejects ambiguous or unidentified providers", async () => {
    ...
    target.ethereum = { request: "not-callable", isMetaMask: true };
    expect(await discoverWallets()).toEqual([]);
