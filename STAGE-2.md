# Data Schema Migration Loss Gate — Stage 2
...
Storage common; domain={}. Base `{old:N(8,Field),new:N(8,Field)}`. Field exact `{id:ID,type:'TEXT'|'INT'|'BOOL'|'ENUM',required:bool,meaning:T(384),values:L(8,ID)}`; ENUM values unique/nonempty; non-ENUM values empty. Response `{mapping:N(8,{old_id:ID,new_id:E(16),transform:'IDENTITY'|'RENAME'|'CAST'|'DROP'}),defaults:L(8,{new_id:ID,value:T(64)})}`. Mapping must include every old ID once; nonempty target must exist and be unique; two-old-to-one-new reject before consensus; DROP requires empty new_id, others nonempty. Every new field not targeted: if required needs one default of valid type, else default optional. Targeted field cannot also default. No duplicate defaults. Default parses exactly: TEXT raw≤64 bytes; INT `-?(0|[1-9][0-9]*)` magnitude≤2^31-1; BOOL true/false literal; ENUM an allowed value. Caps base8192,response4096.

Complete structural truth table, required and optional old fields treated identically for information preservation: DROP always LOSS; defaulting cannot recover dropped optional values. IDENTITY requires same ID; RENAME requires different ID; both require equal type and, for ENUM, new values superset old values. CAST permits ENUM→TEXT (lossless injective encoding), ENUM→ENUM superset, or same-type non-ENUM identity encoding; every other cast LOSS. TEXT→ENUM, INT→TEXT, TEXT→INT, BOOL→INT are unsupported/lossy regardless of plausible examples. Old optional→new required without default for absent source is LOSS (defaults are for untargeted fields only); old required→optional is allowed. Valid newly required untargeted default is not loss of old data; missing one rejects input. Meaning DIFFERENT adds LOSS even when structure allows; UNKNOWN→UNRESOLVED globally. Result `{v:1,meanings:N(8,'SAME'|'DIFFERENT'|'UNKNOWN'|'SKIP_DROP')}` fixed old order. DROP rows must be SKIP_DROP and non-DROP rows forbid it; do not ask semantic equivalence to a missing target. A SKIP_DROP row still deterministically causes LOSS. Any unknown→UNRESOLVED; otherwise any table/meaning loss→LOSS_FOUND; else LOSSLESS. Empty mapping cannot mean success.

| Public signature | Caller/profile | Postcondition |
|---|---|---|
| create_schema_case(nonce:str,mapper:Address,base_json:str,parent:u256)->u256 | any/CREATE distinct | BASE_DRAFT |
| replace_schemas(id:u256,base_json:str,expected_revision:u256)->None | primary/EDIT_BASE | old/new replaced |
| lock_schemas(id:u256,expected_revision:u256)->None | primary/LOCK_BASE | BASE_LOCKED |
| put_mapping(id:u256,response_json:str,expected_revision:u256)->None | secondary/PUT_RESPONSE | RESPONSE_DRAFT |
| freeze_mapping(id:u256,expected_revision:u256)->None | secondary/LOCK_RESPONSE | FROZEN |
| evaluate_migration(id:u256,expected_revision:u256)->None | any/EVALUATE | DONE LOSSLESS/LOSS_FOUND or UNRESOLVED |
| retry_migration(id:u256,expected_revision:u256)->None | any/RETRY | common |

UI owner two schema tables with type/required/values/meaning; mapper mapping rows for every old ID and default rows; disable duplicate targets but show contract error too. Result loss rows generated by deterministic reducer plus meanings vector. Copy `Lossless for the declared record schema and mapping; no migration executed.` Tests cartesian required×transform×type/enum cases, optional DROP, narrowing CAST, missing default, malformed BOOL default, duplicate target, semantic different/unknown, every API proof/recovery row.

WRONG:
```python
lossless = not old_field['required']  # optional DROP accepted
```
CORRECT:
```python
if transform == 'DROP': loss = True  # optional values can still carry information
```
Symptom silent optional data loss; cause requiredness mistaken for informational value; verify `pytest tests/test_contract.py -k migration_table`; fixed when optional/required drop both LOSS_FOUND and all table cells have asserted outcomes.
